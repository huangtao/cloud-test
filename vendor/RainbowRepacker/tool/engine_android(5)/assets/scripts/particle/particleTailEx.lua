...
--ParticleTailEx --粒子类
ParticleTailEx = class(ParticleBase);

ParticleTailEx.liveTime = 6;    --粒子生命时长

ParticleTailEx.init = function(self, len, index, node)
    local moredata = node:getMoreData();
    self.m_fade = (math.random()*100)/1000.0 +0.05;--衰减速度
    self.m_active  = true;
    self.m_live = moredata.liveTime or ParticleTailEx.liveTime;--粒子生命
    local h = moredata.h;
    local w = moredata.w;
   ...
   if direction==1 then
    ...
    elseif direction==2 then
        self.m_yi = -math.random(5);
        self.m_xi = (5-math.random(10))*0.01;
        self.m_x = (20-math.random(40));
        self.m_y = -math.random(30);
    ...
    --self.m_y = 0;
    self.m_scale = 1.0;
    self.m_scale = (5 + math.random(5))*0.1*self.m_scale;
    self.m_tick = 0;
...
local Cos = math.cos
local Sin = math.sin
ParticleTailEx.update = function(self)
