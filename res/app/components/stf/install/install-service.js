...
, StorageService
  ...
  installService.uploadFile = function(control, $files) {
    var resolver = Promise.defer()
    var installation = new Installation('uploading')
    $rootScope.$broadcast('installation', installation)
    StorageService.storeFile('apk', $files, {
      filter: function(file) {
        return /\.apk$/i.test(file.name)
      }
    })
      .progressed(function(e) {
        ...
        var fileinfo = res.data.resources.file
        resolver.resolve(fileinfo)
        /*installation.href = res.data.resources.file.href
        $http.get(installation.href + '/manifest')
          .then(function(res){
            if(res.data.success){
              installation.manifest = res.data.manifest
              var fileinfo = {
              ...
              var err = new Error('Unable to retrieve manifest')
              resolver.reject(err)
            }
          })*/
      })
