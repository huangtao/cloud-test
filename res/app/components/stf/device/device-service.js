...
module.exports = function DeviceServiceFactory($http, socket, EnhanceDeviceService) {
  ...
  function Tracker($scope, options) {
    ...
    function sync(data) {
      ...
      data.usable = data.present && data.status === 3 && data.ready &&
        (!data.owner || data.using)&& !data.isUserUseOther

      if(data.isUserUseOther){
  ...
  deviceService.updateNote = function(serial, note) {
	...
	deviceService.updateAssetNum = function(serial, assetNum) {
		socket.emit('device.assetNum', {
			serial: serial,
			assetNum: assetNum
		})
	}
	...
	deviceService.checkUsedDevice = function($scope){
		var tracker = deviceService.trackAll($scope)
		var devices = tracker.devices
		
		return devices
//      return $http.get('/app/api/v1/checkUsedDevice')
//        .then(function(response) {
