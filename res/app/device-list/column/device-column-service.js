...
function DeviceNameCell(options) {
  ...
  , update: function(td, device) {
      var a = td.firstChild
      var t = a.firstChild
      //add by SissiWu @2016.06.07 start
      var serial = device.serial
      //add by SissiWu @2016.06.07 end
      ...
      if (device.using) {
        ...
        //modify by SissiWu @2016.06.07 start
        //a.href = '#!/control/' + device.serial
//        checkDeviceStatus()
        //modify by SissiWu @2016.06.07 end
      } else {
...
function DeviceStatusCell(options) {
  ...
  , update: function(td, device) {
      ...
      var serial = device.serial
      //add by SissiWu @2016.06.07 end
      a.className = 'btn btn-xs device-status ' + (stateClasses[device.state] || 'btn-default-outline')
...
//    	  a.onclick = function (){
//        	  if (confirm('确定使用这台设备？')){
//        		  window.location.href = '#!/control/' + serial
//        	  }
//    	  }
    	  //modify by SissiWu @2016.06.07 end
      } else {
...
function AssetNumCell(options) {
	return _.defaults(options, {
		...
		, build: function() {
			...
			td.className = 'device-asset-num'
			span.className = 'xeditable-wrapper'
			span.appendChild(document.createTextNode(''))

			i.className = 'device-asset-num-edit fa fa-pencil pointer'
