...
, StandaloneService
  ...
  , link: function(scope, element) {
      ...
      function checkUsedDevice(device){
    	  var tmp_devices = tracker.devices
    	  var used_devices = []
    	  var cur_user_name = scope.currentUser.name
    	  for (i in tmp_devices){
    		  var tmp_device = tmp_devices[i]
    		  var state = tmp_device.state
    		  if (state == 'using'){
    			  if (tmp_device.owner.name == cur_user_name){
    				  used_devices.push(tmp_device)
    			  }
    		  }
    	  }
    	 /* if (used_devices.length >= 1){
    		  alert('用户远程租用真机个数不能超过1台')
    	  } else {
    		  if (confirm('确定使用这台设备？')){
    			  window.location.href = '#!/control/' + device.serial
    		  }
    	  }*/
      }
