/**
 * Created by YoungLiu on 2017/12/1.
 */
...
process.on('message', function (m) {
  switch (m.task){
    ...
    case 'install':
      ...
      installInfo = m.rd
      installFinish = true
      log.debug('travel test install rpldata err and rd:', installErr, installInfo)
      break
    case 'pullScreenCap':
...
function test(options, taskid) {
  ...
  var luaviewerHelperPkg = 'com.boyaa.luaviewer_helper'
  ...
  log = logger.createLogger('travelTest:'+taskid+':'+serial)
  ...
  testutil.createadb({host:host}, serial, log)
      ...
      .then(function () {
        //安装luaviewer_Helper.apk
        log.debug('start install luaviewer_Helper.apk')
        var luaHelperPath = '/var/stf/vendor/luaviewer_helper/luaviewer_helper.apk'
        return testutil.installApk(serial, luaHelperPath, adb, log )
      })
      .then(function () {
        //安装被测apk
        ...
        return new Promise(function (resolve, reject) {
          ...
          exec('mv '+runReportPath+'* '+reportPath, function (err, stdout, stderr) {
            if(err){
              ...
              log.debug('print parsereport')
              console.log(parseResult)
              if(parseResult.error){
                rpldata.statusCode = 5
                rpldata.result = {errorType: 'travel test failed', reson: parseResult.error}
              }else{
        ...
        return new Promise(function (resolve, reject) {
          ...
          function getScreen(cb){
            if(rpldata.cases && rpldata.cases.length > casesIndex){
              ...
              var lsCmd = 'ls -rt ' + reportScreenPath
              log.debug('print nameArr')
              console.log(nameArr)
              log.debug('print reportScreenPath')
              console.log(reportScreenPath)
              log.debug('print lsCmd')
              console.log(lsCmd)
              exec(lsCmd, function (err, stdout, stderr) {
                log.debug('print ls-rt result')
                console.log(stdout)
      ...
      .finally(function () {
        ...
        adb.shell(serial, startuiautomatorCmd)
            ...
            .then(function () {
              log.debug('uninstall apk ', luaviewerHelperPkg)
              return adb.uninstall(serial, luaviewerHelperPkg)
                  .then(function (err) {
                    if (err && !(/true/.test(err))) {
                      log.error('uninstall failed ', luaviewerHelperPkg, err)
                    } else(
                        log.debug('uninstall pkg finish. ')
                    )
                  })
