  ...
  testutil.createadb({host:host}, serial, log)
    ...
    .then(function () {
      ...
      .then(function () {
        ...
        luaHelperPath = '/var/stf/vendor/luaviewer_helper/luaviewer_helper.apk'
    ...
    .then(function () {
      ...
      .then(function () {
        //开启对应手机的tcpip网络调试模式，开启后主服务器才能connect子服务器上的手机
        return adb.tcpip(serial)
      })
      .then(function () {
        //获取子服务器上对应手机的ip
    ...
    .then(function (ip) {
      ...
      var args = util.format('luaport:%s,host:%s,devicename:%s,apppackage:%s,appactivity:%s，deviceip：%s'
      , appiumPort, host, serial, pkg, launchActivity, ip)
