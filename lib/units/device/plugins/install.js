...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push) {
    ...
    router.on(wire.InstallMessage, function(channel, message) {
      ...
      function pushApp() {
        var reURI = message.href.split('/')
        var URIhref= '/'+reURI[1]+'/'+reURI[2]+'/'+reURI[3]+'/'+encodeURIComponent(reURI[4])
        //支持中文apk安装
        var req = request({
          url: url.resolve(options.storageUrl,URIhref)
        })
