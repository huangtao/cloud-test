...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, properties, abi, sdk) {
    ...
    var specialSerials = ['741AEDQR2FMCR']
    if(specialSerials.indexOf(options.serial) != -1){
      abi.all.shift()
    }
    log.debug('abi.all : ' + abi.all)
    var resources = {
      bin: new Resource({
    ...
    return stop()
      ...
      .then(function() {
        ...
        , run: function(cmd) {
            log.debug(util.format('LD_LIBRARY_PATH=%s exec %s %s', path.dirname(resources.lib.dest), resources.bin.dest, cmd))
