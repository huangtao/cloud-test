/**
 * Created by GuixiangGui on 2017/8/11.
 */
...
var parsereport = require('../autoui/parsereport')
var parseperformance = require('./parsePerformance')
...
function test(options, taskid) {
  ...
  var performanceFilePath = uiLogPath + 'performanceFile.cvs'
  ...
  var pMonitorPkg = 'com.boyaa.stf.pmonitor'
  ...
  var rpldata = {result:{}}
  rpldata.performance = {launchTime:''}
  ...
  var game_id = options.moduleId


  var apkLaunchStartTime
  var apkLaunchEndTime
  var launchApkStartRegExp = new RegExp('am.*start.*-W.*'+pkg, 'i')
  var launchApkEndRegExp = new RegExp("ELEMENT: '1'", 'i')
  ...
  rpldata.logs = {
    ...
    ,'report' : reportPath
    ,'logs' : logsPath
    ,'performanceFile' : performanceFilePath
  }

  log = logger.createLogger('performanceTest:'+taskid+':'+serial)

  var performanceFilePhonePath = '/mnt/sdcard/'+pkg+'_result.csv'
  var performanceScreencapOnphonePath = '/mnt/sdcard/minicap_screen/'
  var screenCapNames = []
  ...
  testutil.createadb({host:host}, serial, log)
    ...
    .then(function(){
      return adb.shell(serial, 'rm -r /mnt/sdcard/.'+pkg)
        .catch(function(err){
          log.debug('delete apk sdcard cache file failed. err:' + err)
        })
    })
    .then(function(){
      adb.shell(serial, 'rm -r '+performanceScreencapOnphonePath+'*')
        .catch(function(err){
          log.debug('delete phone screen failed. err:' + err)
        })
      return adb.shell(serial, 'rm -r '+performanceFilePhonePath)
        .catch(function(err){
          log.debug('delete apk sdcard cache file failed. err:' + err)
        })
    })
          ...
          return Promise.promisify(cursor.toArray, cursor)()
            .then(function (cases) {
              ...
              if(cases && cases.length > 0){
                ...
                // srcPath = '/var/stf/testproject2/性能项目/统一大厅常规checklist/1600/DFQP/src/'
    ...
    .then(function(){
      //获取手机上截图的名字
      adb.shell(serial, 'ls ' + performanceScreencapOnphonePath)
        .then(adbkit.util.readAll)
        .then(function(output){
          var data = output.toString().trim()
          if(data.length>0 && !(/No such file/i.test(data))){
            screenCapNames = data.split(/\s+|\r\n|\r|\n/)
          }
        })

      return new Promise(function(resolve, reject){
        adb.pull(serial, performanceFilePhonePath)
          .then(function(transfer){
            transfer.on('end', function(){
              resolve('')
            })

            transfer.pipe(fs.createWriteStream(performanceFilePath))
          })
          .catch(function(err){
            log.warn('performance file not exist. ' + err)
            resolve('')
          })
    ...
    .then(function(){
      return new Promise(function(resolve, reject){
        fs.stat(performanceFilePath, function(err, stats){
          if(err){
            return resolve('')
          }
          if(stats && stats.isFile()){
            return parseperformance(performanceFilePath, rpldata)
              .catch(function(err){
                log.debug('parser performance file error. err:'+err.toString())
              })
              .finally(function(){
                return resolve('')
              })
    ...
    .then(function(){
      log.debug('apklaunch start and end time . ', apkLaunchStartTime, apkLaunchEndTime)
      if(apkLaunchStartTime && apkLaunchEndTime){
        rpldata.performance.launchTime = (apkLaunchEndTime - apkLaunchStartTime).toString()
      }
    ...
    .then(function(){
      ...
      return new Promise(function(resolve, reject){
        var option = {
          ...
          , fileOnPhonePath: performanceScreencapOnphonePath
          , fileOnPcPath: reportScreenPath
          , serial: serial
        }
    ...
    .then(function(){
      return new Promise(function(resolve, reject){
        exec('ls', {cwd:reportScreenPath}, function(err, stdout, stderr){
          var screenFileNames = stdout.trim().split(/\s+/)
          for(var i=0; i<rpldata.performanceDetail.screen.length; i++){
            var fileExist = false
            if(rpldata.performanceDetail.screen[i]){
              screenNRegExp = new RegExp(rpldata.performanceDetail.screen[i])
              for(var j=0; j<screenFileNames.length; j++){
                if(screenNRegExp.test(screenFileNames[j])){
                  rpldata.performanceDetail.screen[i] = reportScreenPath + screenFileNames[j]
                  fileExist = true
                  break
                }
              }
              if(!fileExist){
                rpldata.performanceDetail.screen[i] = ''
              }
            }
          }
          resolve('')
    ...
    .finally(function () {
      ...
      adb.shell(serial, startuiautomatorCmd)
        ...
        .then(function(){
          ...
          log.debug('rpldata performance. ', rpldata.performance, rpldata.performanceScenes, rpldata.performanceDetail)
  ...
  function startAppium(path, p, lPath, apppkg, appactivity) {
    ...
    function start(appiumPath, port, logPath, pkg, activity, cb) {
      ...
      getAvailablePort(port)
        ...
        .then(function () {
          var getApkLaunchStartTime = true
          var getApkLaunchEndTime = true
          ...
          function execCmd(cmd){
            ...
            .then(function (startAppiumStream) {
              ...
              startAppiumStream.on('data', function (data) {
                ...
                if (getApkLaunchStartTime && launchApkStartRegExp.test(data.toString())) {
                  var dataList = data.toString().split(/\r\n|\r|\n/)
                  for(i in dataList){
                    if(launchApkStartRegExp.test(dataList[i])){
                      apkLaunchStartTime = Date.parse(dataList[i].split('- [')[0].trim())
                      getApkLaunchStartTime = false
                      break
                    }
                  }
                }

                if (getApkLaunchEndTime && launchApkEndRegExp.test(data.toString())) {
                  var dataList = data.toString().split(/\r\n|\r|\n/)
                  for(i in dataList){
                    if(launchApkEndRegExp.test(dataList[i])){
                      apkLaunchEndTime = Date.parse(dataList[i].split('- [')[0].trim())
                      getApkLaunchEndTime = false
                      break
                    }
                  }
                }
  ...
  function installAppiumNeedApks() {
    ...
    var pMonitorPath = '/var/stf/vendor/performanc/pmonitor.apk'
    ...
    return adb.isInstalled(serial, appiumImePkg)
      ...
      .then(function(){
        return adb.isInstalled(serial, pMonitorPkg)
      })
      .then(function(isinstalled){
        if(!isinstalled){
          log.debug('install pmonitor apk')
          return testutil.installApk(serial, pMonitorPath, adb, log)
        }
      })
      .catch(function(){
        return
