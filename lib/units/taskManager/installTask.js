    ...
    var installLoop = setInterval(function(){
      if(installTaskFinish){
        ...
        if(taskI){
          ...
          if(clearIntervalLoopCount > 2){
            clearInterval(installLoop)
            clearIntervalLoopCount = 0
          }
      ...
      testutil.wakeUpPhone(serial, adb, log)
        .then(function () {
          ...
          adb.install(serial, apkPath)
            ...
            .finally(function () {
              if(error){
                var screenPath = '/data/local/tmp/installFailScreen.jpg'
                rpldata.installFailScreen = screenPath
                testutil.screenCap(serial, adb, screenPath)
                  .timeout(10000)
                  .finally(function(){
                    cb(error, rpldata)
