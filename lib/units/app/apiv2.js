...
var hitCount = tasks.length
    ...
    target_tasks.forEach(function (task) {
      if (task.taskType == 'uiTest' || task.taskType == 'performTest' || task.taskType == 'travelTest') {
        // 只对未完成的
...
router.post('/rerun', function (req, res) {
  ...
  dbapi.loadFailTestCaseIds(taskId, serial).then(function (taskInfo) {
    ...
    if (target_ids.length) {
	  ...
	  else if(task.tasktype == 'performTest') {
		performtest(req, taskid)	
	  }
	  else if(task.tasktype == 'monkey') {
		monkey(req, taskid, 'apiv2')	
	  }
	  else if(task.tasktype == 'activityTest') {
		activityCompatibility(req, taskid)
	  }
      else if(task.tasktype == 'travelTest'){
