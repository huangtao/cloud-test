        ...
        function delFile(fileName){
          ...
          fs.stat(fileName, function (err, stats) {
            if (stats && stats.isFile()) {
              ...
              if (/cptlog/i.test(fileName)) {
                exec('rm -r ' + fileName + '/' + serial + '*')
              } else {
                exec('rm -r *', {cwd: fileName})
              }
...
testutil.befortest = function (serial, adb, log) {
  ...
  return adb.shell(serial, 'settings get secure default_input_method')
    ...
    .then(function () {
      return testutil.startUiautomator(serial, adb, log)
    })
    .then(function () {
      ...
      if (pkgRegExp.test(outputStr)) {
        ...
        for (var i = 0; i < psArr.length; i++) {
          if (pkgRegExp.test(psArr[i])) {
            log.debug(psArr[i])
            killpid(psArr[i].split(/\s+/))
            
            }

        }
        
        function killpid(lineArr){
...
testutil.startUiautomator = function(serial, adb, log){
...
testutil.screenCap = function(serial, adb, path){
  var screenCapPreCmd =  'LD_LIBRARY_PATH=/data/local/tmp exec /data/local/tmp/minicap -P '
  return adb.stat(serial, '/data/local/tmp/minicap')
    .timeout(5000)
    .then(function(stats){
      if ((stats.mode & 0x40) !== 0x40){
        screenCapPreCmd = 'LD_LIBRARY_PATH=/data/data/com.android.shell exec /data/data/com.android.shell/minicap -P '
      }
      console.log(serial, screenCapPreCmd)
    })
    .then(function(){
    ...
    .then(function(displayStr){
      resolutionReal = displayStr.toString().match(/mOverrideDisplayInfo.*/i)[0].match(/real\s*\d*\s*x\s*\d*/i)[0]
        .match(/\d*\s*x\s*\d*/i)[0].replace(/\s/g, '')
      
      var screencapCmd = screenCapPreCmd + resolutionReal + '@' + resolutionReal + '/0 -s >' + path
      console.log(serial, screencapCmd)
